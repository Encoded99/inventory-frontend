...
const inputPackages=document.querySelectorAll('.package')
  ...
  const quantity=   Array.from(inputQuantity).map((input)=>parseFloat(input.value)).filter((numbers)=>numbers!==0)
...
const handleSubmit = async () => {
  ...
  if (status){
    ...
    setMessage(`Sales recorded successfully for item  ${i+1}`);
    ...
    setMessage(`Error recording sales for item ${i+1}`);
          ...
          if (sentData===costData.length){
            ...
            if (successfulSalesCount===costData.length){
              ...
              setApiError(false)
              setTimeout(()=>{
                setMessage('All sales recorded successfully');
             ...
             setIsRecordFailed(true)
              setApiError(true)
              setTimeout(()=>{
                setMessage(`${failedRequest} uploads failed` );
        ...
        <div className='confirmation-container error-container'>
...

          {
            errorData.map((item, index) => {
              return (
                <div key={index} className='error-info'>
...
{
  (isFinish && successData.length!==0) && (
    <>
    
    <div className='confirm-container-overlay' style={{ zIndex: '2' }}>
...
<p style={{fontSize:'1.0em',margin:'2%',fontWeight:"bold",color:"#777",textAlign:'center'}}>RECEIPT</p>
...
<div  className='total-cost-div'>Total Cost(&#x20A6;):  <strong>       {totalSuccessCost}</strong>  </div>
