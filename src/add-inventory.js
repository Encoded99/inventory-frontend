...
<div className={apiError === true ? 'alert alert-danger' : apiError === false ? 'alert alert-success' : ''} style={{fontWeight:'700',margin:"2vh 0"}}>
  {loadingMessage}
   ...
   <p style={{fontSize:'2em',textAlign:'center',margin:'0.0em 0'}}>Add  Inventory </p> <hr />
...
<section className='register-info-container' style={{boxShadow:'',backgroundColor:""}}>
...
<div style={{width:"100%",display:'grid',gridTemplateColumns:"1fr 1fr",gridGap:'4%'}}>
<div>Name</div>
{specificData && specificData.length > 0 ? (
 

<strong>{specificData[0].name.charAt(0).toUpperCase() + specificData[0].name.slice(1).toLowerCase()}</strong>
) : (
  <p></p>
)}
...
<strong>{specificData[0].sku}</strong>
) : (
  <p></p>
)}
...
<strong>{specificData[0].upb}</strong>
) : (
  <p></p>
)}
...
<div style={{width:"100%",display:'grid',gridTemplateColumns:"2fr 1fr",gridGap:'4%'}}>
<div> Current Price Bulk (&#x20A6;)</div>
{specificData && specificData.length > 0 ? (
 

<strong>{specificData[0].currentPrice.bulkPrice}</strong>
) : (
  <p></p>
)}
...
<strong>{specificData[0].currentPrice.piecesPrice}</strong>
) : (
  <p></p>
)}
 ...
 onKeyDown={(e) => {
   ...
   if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
     e.preventDefault();
      ...
      <div className='register-data-container'>
    <p  className='register-text' style={{marginRight:''}}>Delivery Date </p>
